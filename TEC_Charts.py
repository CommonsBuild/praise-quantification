...
df1.dropna(subset = ["Impact Hours"], inplace=True)
df2 = pd.read_csv('TEC#3_IH.csv')

fig1 = px.bar(df1.sort_values(by=['Impact Hours'], ascending=True), x="Impact Hours", y="Telegram Handle", orientation='h', width=300)
fig1.update_layout(autosize=False, width=1400, height=2000)
